    ...
    if (!checkConnection()) {
      ...
      try {
        ...
        const cities = Array.isArray(data)
          ? data.map(c => ({ _id: c._id || c.id || c._doc?._id, name: c.name }))
          : []
    ...
    if (!checkConnection()) {
      ...
      try {
        ...
        const bars = Array.isArray(data)
          ? data.map(b => ({ _id: b._id || b.id || b._doc?._id, name: b.name, city: b.city }))
          : []
    ...
    if (!checkConnection()) {
      ...
      try {
        ...
        const roles = Array.isArray(data)
          ? data.map(r => ({ _id: r._id || r.id || r._doc?._id, name: r.name, displayName: r.displayName || r.name }))
          : []
    ...
    if (!checkConnection()) {
      ...
      try {
        ...
        const brands = Array.isArray(data)
          ? data.map(b => ({ _id: b._id || b.id || b._doc?._id, name: b.name, displayName: b.displayName || b.name }))
          : []
