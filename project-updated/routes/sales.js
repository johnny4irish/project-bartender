...
router.get('/stats', auth, async (req, res) => {
  try {
    ...
    // Calculate monthly rank (Mongo aggregate, approved sales in current month)
    let monthlyRank = 0;
    try {
      const monthlyAgg = await Sale.aggregate([
        { $match: { verificationStatus: 'approved', createdAt: { $gte: currentMonth } } },
        { $group: { _id: '$user', monthlyAmount: { $sum: { $ifNull: ['$price', 0] } }, monthlySales: { $sum: 1 } } },
        { $sort: { monthlyAmount: -1 } }
      ]);

      const userIndex = monthlyAgg.findIndex(u => u._id?.toString() === userId.toString());
