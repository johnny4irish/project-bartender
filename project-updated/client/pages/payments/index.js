...
const PaymentsPage = () => {
  ...
  const fetchBalance = async () => {
    try {
      const data = await apiCall('/api/payments/balance')
      setBalance(data)
    } catch (error) {
      console.error('Ошибка загрузки баланса:', error)
      toast.error(error.message || 'Ошибка загрузки баланса')
    } finally {
  ...
  const fetchTransactions = async () => {
    try {
      ...
      const data = await apiCall(`/api/payments/transactions?${params.toString()}`)
      setTransactions(data.transactions)
      setTransactionStats(data.stats)
      setTotalPages(data.pagination.pages)
    } catch (error) {
      console.error('Ошибка загрузки транзакций:', error)
      toast.error(error.message || 'Ошибка загрузки транзакций')
    } finally {
...
// Компонент модального окна вывода средств
const WithdrawModal = ({ balance, onClose, onSuccess }) => {
  const [amount, setAmount] = useState('')
  const [phone, setPhone] = useState('')
  const [submitting, setSubmitting] = useState(false)
  const minAmount = balance?.minWithdrawAmount || 0
  const maxAmount = balance?.maxWithdrawAmount || 0
  const commissionRate = balance?.commissionRate || 0

  const formattedReceive = () => {
    const num = Number(amount)
    if (!num || num <= 0) return 0
    const commission = Math.round(num * commissionRate)
    return Math.max(0, num - commission)
  }
  ...
  const submit = async (e) => {
    ...
    if (num < minAmount) {
      toast.error(`Минимальная сумма: ${minAmount} ₽`)
      return
    }
    if (num > maxAmount) {
      toast.error(`Максимальная сумма: ${maxAmount} ₽`)
      return
    }
    ...
    setSubmitting(true)
    try {
      await apiCall('/api/payments/withdraw', {
        method: 'POST',
        body: JSON.stringify({ amount: num, phoneNumber: phone })
      })
      toast.success('Запрос на вывод создан')
      onSuccess && onSuccess()
    } catch (error) {
      toast.error(error.message || 'Не удалось создать вывод')
    } finally {
      setSubmitting(false)
    }
  }
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg p-6 max-w-md w-full mx-4">
        ...
        <form onSubmit={submit} className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Номер телефона (СБП)</label>
            <input
              type="tel"
              value={phone}
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Сумма вывода</label>
            <input
              type="number"
              value={amount}
          ...
          <div className="bg-blue-50 border border-blue-200 rounded-md p-3 text-sm text-blue-800">
            Комиссия: {(commissionRate * 100).toFixed(0)}% • Вы получите: <strong>{formattedReceive().toLocaleString()} ₽</strong>
          </div>
          <div className="flex justify-end space-x-3">
            <button type="button" onClick={onClose} className="bg-gray-500 hover:bg-gray-600 text-white px-4 py-2 rounded-lg">Отмена</button>
            <button type="submit" disabled={submitting} className="bg-gray-900 hover:bg-gray-800 text-white px-4 py-2 rounded-lg disabled:opacity-50">
